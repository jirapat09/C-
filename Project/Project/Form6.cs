    ...
    public partial class Form6 : Form
    {
        ...
        private void dataadmin_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                else if (comboBox.Text == "ข้อมูลลูกค้า")
                {
                    dataadmin.CurrentRow.Selected = true;
                    unBox.Text = dataadmin.Rows[e.RowIndex].Cells["username"].FormattedValue.ToString();
                    passBox.Text = dataadmin.Rows[e.RowIndex].Cells["password"].FormattedValue.ToString();
                    teleBox.Text = dataadmin.Rows[e.RowIndex].Cells["telephone"].FormattedValue.ToString();
                    emailBox.Text = dataadmin.Rows[e.RowIndex].Cells["email"].FormattedValue.ToString();
                    addressBox.Text = dataadmin.Rows[e.RowIndex].Cells["address"].FormattedValue.ToString();
                }
        ...
        private void comboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (comboBox.Text == "ประวัติการขาย")
            {
                ...
                MySqlConnection conn = databaseConnection();
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM history WHERE status ='"+"โอนแล้ว(ตรวจสอบแล้ว)"+ "'", conn);
        ...
        private void editbtn_Click(object sender, EventArgs e)
        {
            ...
            int editid = Convert.ToInt32(dataadmin.Rows[selectedRows].Cells["id"].Value);
            MySqlConnection conn = databaseConnection();
            String sql = "UPDATE user SET username = '" + unBox.Text + "',password = '" + passBox.Text + "',telephone = '" + teleBox.Text + "',email = '" + emailBox.Text + "',address = '"+addressBox.Text+"' WHERE id ='" + editid + "'";
            conn.Open();
            MySqlCommand cmd = new MySqlCommand(sql, conn);
            int rows = cmd.ExecuteNonQuery();
            conn.Close();
